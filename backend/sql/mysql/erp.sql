/*
 ...
 说明：
 1. yudao-server 默认注释掉 yudao-module-erp，本项目为了给 ACF / MCP 提供真实只读能力已启用该模块，
    因此必须先执行本脚本创建 erp_* 表，否则应用启动后调用 ERP 能力会报表不存在。
 2. 本脚本内含 DROP TABLE IF EXISTS，会清空已存在的 erp_* 表数据。如果库里已有 ERP 业务数据，
    执行前请先备份，或只挑选缺失的表单独执行。
 3. erp_* 表的菜单与权限数据（erp:product:query 等）已包含在 ruoyi-vue-pro.sql 中，无需另外导入。
 4. 测试数据见同目录 erp_acf_seed_data.sql，可重复执行。
*/
